...
**/
	...
	if (RootPtr)
    {
        ...
        if (!exists && (*temp = malloc(sizeof(**temp))))
        {
            (*temp)->Cont = Content;
            (*temp)->LPtr = NULL;
            (*temp)->RPtr = NULL;
...
**/
    ...
    Leftmost_Postorder(RootPtr, &S, &B);
    while (!SEmptyP(&S))
    {
        node = STopPopP(&S);
        SPushP(&S, node);
        ...
        {
            SPushB(&B, false);
            Leftmost_Postorder(node->RPtr, &S, &B);
        }
        else
        {
            STopPopP(&S);
